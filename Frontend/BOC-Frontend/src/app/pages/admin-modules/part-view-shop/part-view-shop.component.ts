...
@Component({
  selector: 'app-part-view-shop',
  templateUrl: './part-view-shop.component.html',
  styleUrls: ['./part-view-shop.component.scss'],
  providers: [
    BsModalRef,
    BsModalService
  ]
})
export class PartViewShopComponent implements OnInit {
  // AddForm: FormGroup;
  vendorList: any[];
  ...
  loadInventoryView(PartId: any) {
    this.service.postHttpService({ PartId }, 'viewShopPart').subscribe(response => {
      this.viewResult = response.responseData.data;
      this.viewResultItems = response.responseData.dataItems;
