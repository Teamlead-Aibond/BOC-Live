...
@Component({
  selector: 'app-bulk-substatus-assign-edit',
  templateUrl: './bulk-substatus-assign-edit.component.html',
  styleUrls: ['./bulk-substatus-assign-edit.component.scss']
})
export class BulkSubstatusAssignEditComponent implements OnInit {
  Form: FormGroup;
  bulkData: any = [];
  submitted = false;
  adminList: any = []
  ...
  onSubmit() {
    this.submitted = true;
    if (this.Form.valid) {
      var RRIds = (this.bulkData.map(a => a.RRId)).toString();
      var postData = {
        RRIds: RRIds,
