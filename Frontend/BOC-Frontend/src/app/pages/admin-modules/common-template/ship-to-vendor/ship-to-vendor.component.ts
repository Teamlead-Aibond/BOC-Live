...
import { BillTransportationTo, CONST_AH_Group_ID, Const_Alert_pop_message, Const_Alert_pop_title, CONST_ShipAddressType, shipDescription, UPS_AH_Account_No, UPS_ID, UPS_Service } from 'src/assets/data/dropdown';
...
@Component({
  selector: 'app-ship-to-vendor',
  templateUrl: './ship-to-vendor.component.html',
  styleUrls: ['./ship-to-vendor.component.scss']
})
export class ShipToVendorComponent implements OnInit {
  CustomerName;
  VendorName
  ahName;
  ...
  UPS_Receiver_number
  VendorUPSShipperNumber
  CustomerUPSShipperNumber
  ...
  IsUPSEnable
  IsUserUPSEnable
  BillTransportationToList
  CustomerShipIdLocked: any;
  VendorShipIdLocked: any;
  showShipToLocation: any = 0;
  constructor(public modalRef: BsModalRef,
    private fb: FormBuilder,
    private CommonmodalService: BsModalService,
    private datePipe: DatePipe,
    private cd_ref: ChangeDetectorRef,
    private commonService: CommonService, private soapserviceService: SoapserviceService,
  ...
  ngOnInit(): void {
    ...
    this.BillTransportationToList = BillTransportationTo
    ...
    this.ShippingIdentityId = this.data.ShippingIdentityId
    this.ShippingAddressId = this.data.ShippingAddressId
    this.ShippingIdentityName = this.data.ShippingIdentityName
    this.ShippingStatus = this.data.ShippingStatus;
    this.Status = this.data.Status
    this.CustomerShipIdLocked = this.data.CustomerShipIdLocked;
    this.VendorShipIdLocked = this.data.VendorShipIdLocked;

    this.model.ShippedBy = localStorage.getItem("UserName");
    this.ah_groupId = CONST_AH_Group_ID
    if (this.RRShippingHistory.length > 0) {
      this.shippingDetails = this.RRShippingHistory[this.RRShippingHistory.length - 1]
      ...
      this.ShippingIdentityType = this.data.ShippingIdentityType
    }
    //this.model.Address = this.shippingDetails.ReceiveStreetAddress + ',' + this.shippingDetails.ReceiveCity + ',' + this.shippingDetails.ReceiveCountry + ',' + this.shippingDetails.ReceiveState + '-' + this.shippingDetails.ReceiveZip
    ...
    this.ShipAccountNumber = UPS_AH_Account_No
  }
  ...
  getShipFromAddressList() {
    ...
    this.commonService.postHttpService(postData, 'getAddressList').subscribe(response => {
      this.ShipFromAddressList = response.responseData.map(function (value) {
        return { title: value.StreetAddress + " , " + value.City + " , " + value.StateName + " ," + value.CountryName + ".-" + value.Zip, "Address": value.AddressId }
      });
  ...
  setShipVendorAddressList(value) {
    if (this.VendorId == "") {
      ...
      if (value == 0) {
        this.VendorUPSShipperNumber = ""
        var Obj = this
        this.CustomerUPSShipperNumber = ""
        var postData = {
        ...
        // this.CustomerName = ""
        // this.VendorName = this.vendorList.find(a => a.VendorId == this.VendorId).VendorName
        this.showShipToLocation = this.VendorShipIdLocked;
        this.commonService.postHttpService(postData, 'getAddressList').subscribe(response => {
          ...
          var that = this;
          var ShippingAddress = response.responseData.map(function (value) {
            if (that.VendorShipIdLocked > 0) {
              return that.VendorShipIdLocked;
            } else {
              if (value.IsShippingAddress == 1) {
                Obj.VendorUPSShipperNumber = value.VendorUPSShipperNumber
                return value.AddressId
              }
            }
  ...
  setCustomerShipAddress(value) {
    if (value == 1) {
      ...
      // this.VendorName = ""
      // this.CustomerName = this.customerList.find(a => a.CustomerId == this.CustomerId).CompanyName
      this.showShipToLocation = this.CustomerShipIdLocked;
      this.commonService.postHttpService(postData, 'getAddressList').subscribe(response => {
        this.ShipToAddressList = response.responseData.map(function (value) {
          return { title: value.StreetAddress + " " + value.SuiteOrApt + ", " + value.City + " , " + value.StateName + " ," + value.CountryName + ". - " + value.Zip, "ShipToAddressId": value.AddressId }
        });
        ...
        var that = this;
        var ShippingAddress = response.responseData.map(function (value) {
          if (that.CustomerShipIdLocked > 0) {
            console.log(that.CustomerShipIdLocked);
            return that.CustomerShipIdLocked;
          } else {
            if (value.IsShippingAddress == 1) {
              Obj.CustomerUPSShipperNumber = value.CustomerUPSShipperNumber
              return value.AddressId
            }
          }
        });
    ...
    if (value == 0) {
      ...
      this.commonService.getHttpService("getAHGroupVendorAddress").subscribe(response => {
        if (response.status == true) {
          ...
          this.model.ShipToAddressId = this.ShipToAddressList[0].ShipToAddressId
          this.ShipToAddress = response.responseData.AHGroupVendorAddress
          Obj.VendorUPSShipperNumber = this.ShipToAddressList[0].VendorUPSShipperNumber
        }
        else {
  ...
  onSubmit(f: NgForm) {
    ...
    if (f.valid) {
      if (this.model.ShipToAddressId != this.model.Address) {
        ...
        if (this.showVendorName == false) {
          var ShipToName = this.CustomerName
        } else {
          var ShipToName = this.VendorName
        }
        ...
        var UPS_Receiver_number = ''
        if (this.showVendorName == false && this.model.BillToType != 1) {
          UPS_Receiver_number = this.CustomerUPSShipperNumber
        } else if (this.showVendorName == true && this.model.BillToType != 1) {
          UPS_Receiver_number = this.VendorUPSShipperNumber
        }
        this.Form = this.fb.group({
          ...
          BillToType: [this.model.BillToType],
          UPS_Receiver_number: [UPS_Receiver_number]
        })
        ...
        if (this.model.Type == 1 && this.ShipFromId == this.ah_groupId && this.model.ShipViaId == this.UPSId && this.IsUPSEnable == '1' && this.IsUserUPSEnable == 1) {
          if (this.model.Type == 1) {
            ...
            var UPS_RECEIVER_NO = this.CustomerUPSShipperNumber && this.CustomerUPSShipperNumber != '' ? this.CustomerUPSShipperNumber : '';

            if ((this.model.BillToType == 2 && UPS_RECEIVER_NO == '')) {

              Swal.fire({
                title: 'Customer UPS No. is required',
                text: 'If Bill Transportation To is Receiver then Customer should have a valid UPS Account No. Add a valid UPS number in Customer profile to proceed.',
                type: 'error',
                confirmButtonClass: 'btn btn-confirm mt-2'
              });
        ...
        if (this.model.Type1 == 0 && this.ShipFromId == this.ah_groupId && this.model.ShipViaId == this.UPSId && this.IsUPSEnable == '1' && this.IsUserUPSEnable == 1) {
          if (this.model.Type1 == 0) {
            // this.soapserviceService.postHttpService(this.post(this.Form.value), this.endpointurl, '').subscribe(
            //   (response: any) => {
            var UPS_RECEIVER_NO = this.VendorUPSShipperNumber && this.VendorUPSShipperNumber != '' ? this.VendorUPSShipperNumber : '';

            if ((this.model.BillToType == 2 && UPS_RECEIVER_NO == '')) {

              Swal.fire({
                title: 'Vendor UPS No. is required',
                text: 'If Bill Transportation To is Receiver then Vendor should have a valid UPS Account No. Add a valid UPS number in Vendor profile to proceed.',
                type: 'error',
                confirmButtonClass: 'btn btn-confirm mt-2'
              });
            } else {
