...
import { booleanValues, CUSTOMER_GROUP_ID_AMAZON } from 'src/assets/data/dropdown';
...
@Component({
  selector: 'app-total-part-list-store',
  templateUrl: './total-part-list-store.component.html',
  styleUrls: ['./total-part-list-store.component.scss']
})
export class TotalPartListStoreComponent implements OnInit {
  ...
  onTotalPartList() {
    ...
    const that = this;
    var filterData = {
      CustomerGroupId:CUSTOMER_GROUP_ID_AMAZON
    }
    ...
    return {
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('.actionView', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.router.navigate(['/admin/shop-parts-edit'], { state: { PartId: data.PartId,StoreType:CUSTOMER_GROUP_ID_AMAZON } });
        });
        $('.actionView1', row).unbind('click');
        $('.actionView1', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.router.navigate(['/admin/shop-parts-view'], { state: { PartId: data.PartId,StoreType:CUSTOMER_GROUP_ID_AMAZON } });
        });
  ...
  onAddParts(){
    this.router.navigate(['/admin/shop-parts-edit'], { state: { StoreType:CUSTOMER_GROUP_ID_AMAZON  } });
  }
}
