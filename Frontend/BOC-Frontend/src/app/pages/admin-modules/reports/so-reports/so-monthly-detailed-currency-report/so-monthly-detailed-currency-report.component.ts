...
@Component({
  selector: 'app-so-monthly-detailed-currency-report',
  templateUrl: './so-monthly-detailed-currency-report.component.html',
  styleUrls: ['./so-monthly-detailed-currency-report.component.scss']
})
export class SoMonthlyDetailedCurrencyReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/ParticularMonthSOByCustomerWithCurrency';
    const that = this;
    var filterData = { "Month": this.Month, "Year": this.Year, "CurrencyCode": this.CurrencyCode, "CreatedByLocation": this.CreatedByLocation, "ReportCurrencyCode": this.ReportCurrencyCode, "IncludeRR": this.IncludeRR }
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "ParticularMonthSOByCustomerToExcelWithCurrency").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
