...
@Component({
  selector: 'app-so-by-parts',
  templateUrl: './so-by-parts.component.html',
  styleUrls: ['./so-by-parts.component.scss']
})
export class SoByPartsComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/SalesByParts';
    const that = this;
    var filterData = {}
    ...
    this.dtOptions["columns"] = [
      ...
      { data: 'SOId', width: '20%', name: 'SOId', defaultContent: '', orderable: true, searchable: true },
    ...
    this.dataTable = $('#datatable-angular-SoByPart');
  ...
  onFilter(event) {
    ...
    var table = $('#datatable-angular-SoByPart').DataTable();
    table.columns(0).search(this.CustomerId);
    table.columns(5).search(this.dateRequested);
    table.columns(6).search(this.dateRequestedTo);
    table.columns(7).search(this.dueDate);
    table.columns(8).search(this.dueDateTo);
    table.columns(12).search(this.PartId);
    table.columns(13).search(this.Status);
    table.columns(14).search(this.SOType);
    table.draw();
    this._opened = !this._opened;
  }
  onClear(event) {
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getSalesByPartsReportToExcel").subscribe(response => {
      if (response.status == true) {
  ...
  generateExcelFormat() {
    ...
    const title = 'Sales Order By Parts Report';
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = ('Sales Order By Parts Report ' + currentDate + '.xlsx')
