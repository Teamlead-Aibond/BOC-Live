...
import { catchError, distinctUntilChanged, debounceTime, map, switchMap } from 'rxjs/operators';
import { CommonService } from 'src/app/core/services/common.service';
import { ExcelService } from 'src/app/core/services/excel.service';
import { CONST_VIEW_ACCESS, CONST_APPROVE_ACCESS, SalesOrder_Status, SalesOrder_Type, footerlineRight, footerlineLeft, ProcessingHTML } from 'src/assets/data/dropdown';
import { environment } from 'src/environments/environment';
import Swal from 'sweetalert2';
import * as fs from 'file-saver';

@Component({
  selector: 'app-so-monthly-currency-report',
  templateUrl: './so-monthly-currency-report.component.html',
  styleUrls: ['./so-monthly-currency-report.component.scss']
})
export class SoMonthlyCurrencyReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.IsViewEnabled = this.commonService.permissionCheck("MonthlySalesSummaryByCurrency", CONST_VIEW_ACCESS);
    this.DownloadExcelCSVinReports = this.commonService.permissionCheck("MonthlySalesSummaryByCurrency", CONST_APPROVE_ACCESS);
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/SalesByMonthWithCurrency';
    const that = this;
    var filterData = {}
    ...
    return {
      ...
      createdRow: function (row, data, index) {
        ...
        $('.actionView1', row).bind('click', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          that.router.navigate(['/admin/reports/Monthly-SO-Detaliled-Currency-Report'], { state: { Month: data.Month, Year: data.Year, ReportCurrencyCode: that.ReportCurrencyCode, IncludeRR: that.IncludeRR } });
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('.actionView1', row).bind('click', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          this.router.navigate(['/admin/reports/Monthly-SO-Detaliled-Currency-Report'], { state: { Month: data.Month, Year: data.Year, ReportCurrencyCode: that.ReportCurrencyCode, IncludeRR: that.IncludeRR  } });
  ...
  onExceFromList(Month, Year) {
    ...
    this.commonService.postHttpService(postData, "SODetailedReportWithCurrency").subscribe(response => {
      if (response.status == true) {
  ...
  generateSOdetailsExcelFormat() {
    ...
    //["SO No", "Part No", "RR # / MRO #", "Company Name", "Date Requested", "Quantity", "Currency", "Price", "Rate", "Discount", "Ext Price", "Ext Cost", "Profit", "GP %"]
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "SalesByMonthReportToExcelWithCurrency").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
