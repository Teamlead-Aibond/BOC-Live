...
export class SoMonthlyReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/SalesByMonth';
    const that = this;
    var filterData = {}
    ...
    return {
      ...
      createdRow: function (row, data, index) {
        ...
        $('.actionView1', row).bind('click', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          that.router.navigate(['/admin/reports/Monthly-SO-Report'], { state: { Month: data.Month, Year: data.Year, IncludeRR: that.IncludeRR  } });
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('.actionView1', row).bind('click', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          this.router.navigate(['/admin/reports/Monthly-SO-Report'], { state: { Month: data.Month, Year: data.Year, IncludeRR: that.IncludeRR  } });
  ...
  onExceFromList(Month, Year) {
    ...
    this.commonService.postHttpService(postData, "SODetailedReport").subscribe(response => {
      if (response.status == true) {
  ...
  generateSOdetailsExcelFormat() {
    ...
    const header = ["SO No", "Part No", "RR # / MRO #", "Company Name", "Date Requested", "Quantity", "Price", "Rate", "Discount", "Ext Price", "Ext Cost", "Profit", "GP %"]
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getSalesByMonthReportToExcel").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
