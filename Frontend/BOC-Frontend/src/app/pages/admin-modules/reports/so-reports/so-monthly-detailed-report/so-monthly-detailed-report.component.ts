...
export class SoMonthlyDetailedReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/ParticularMonthSOByCustomer';
    const that = this;
    var filterData = { "Month": this.Month,"Year":this.Year, "IncludeRR": this.IncludeRR }
  ...
  onExcel() {
    ...
    this.spinner= true
    this.commonService.postHttpService(postData, "ParticularMonthSOByCustomerToExcel").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
  ...
  generateExcelFormat() {
    ...
    const header = ["Customer", "Price", "Cost", "Profit", "Margin"]
    //Create workbook and worksheet
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Data');
    //Add Row and formatting
