...
import { Invoice_Type, Invoice_Status, footerlineRight, footerlineLeft, PurchaseOrder_Status, PurchaseOrder_Type } from 'src/assets/data/dropdown';
import { environment } from 'src/environments/environment';
import Swal from 'sweetalert2';
import * as fs from 'file-saver';

@Component({
  selector: 'app-po-monthly-detailed-report',
  templateUrl: './po-monthly-detailed-report.component.html',
  styleUrls: ['./po-monthly-detailed-report.component.scss']
})
export class PoMonthlyDetailedReportComponent implements OnInit {
    ...
    this.dtOptions["columns"] = [
      {
        ...
        render: (data: any, type: any, row: any, meta) => {
          var link = '';
          link += `<a href="#" class="text-primary actionView1" ngbTooltip="Invoice By Customer">${row.VendorName}</a>`;
          return link;
        }
      },
      ...
      {
        data: 'VendorId', name: 'VendorId', className: 'text-center', orderable: true, searchable: true,
        render: (data: any, type: any, row: any, meta) => {
          var link = '';
    ...
    return {
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('.actionView2', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.onExceFromList(data.VendorId)
        });
  ...
  generateInvoicedetailsExcelFormat() {
    ...
    const title = `${this.Month} Detailed PO Report`;
    const header = ["PartNo", "CAT", "PONO", "SONo", "WO", "VendorName", "InvoiceDate", "Quantity", "Rate", "Price", "Discount", "ExtPrice", "ExtCost", "Profit", "GP", "SerialNo"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = (`${this.Month} Detailed PO Report` + currentDate + '.xlsx')
