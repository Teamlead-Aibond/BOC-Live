...
import { footerlineRight, footerlineLeft, PurchaseOrder_Type, PurchaseOrder_Status } from 'src/assets/data/dropdown';
import Swal from 'sweetalert2';
import { Router } from '@angular/router';
import { NgTranscludeDirective } from 'ngx-bootstrap';
@Component({
  selector: 'app-monthly-po-report',
  templateUrl: './monthly-po-report.component.html',
  styleUrls: ['./monthly-po-report.component.scss']
})
export class MonthlyPoReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/POReports/PurchasesByMonth';
    const that = this;
    var filterData = {}
    ...
    this.dtOptions["columns"] = [
      {
        data: 'Month', width: '10%', name: 'Month', defaultContent: '', orderable: true, searchable: true,
        render: (data: any, type: any, row: any, meta) => {
          var link = '';
          link += `<a href="#" class="text-primary actionView1" ngbTooltip="PO By Vendor">${row.Month}</a>`;
          return link;
        }
      },
      ...
      { data: 'POType', width: '20%', name: 'POType', defaultContent: '', orderable: true, searchable: true },
      ...
      { data: 'Margin', width: '20%', name: 'Margin', defaultContent: '', orderable: true, searchable: true },
      {
        ...
        render: (data: any, type: any, row: any, meta) => {
          var link = '';
          link += '<a href="#" class="far fa-file-excel text-primary actionView2" ngbTooltip="Download Excel" ></a> &nbsp;';
          return link;
        }
      },
    ];

    this.dataTable = $('#datatable-angular-POByMonthly');
    ...
    return {
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('.actionView2', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.onExceFromList(data.Month)
        });
        ...
        $('.actionView1', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          this.router.navigate(['/admin/reports/Monthly-PO-Report'], { state: { Month: data.Month } });
  ...
  onFilter(event) {
    var table = $('#datatable-angular-POByMonthly').DataTable();
    ...
    table.columns(9).search(this.POType);
    table.draw();
    this._opened = !this._opened;
  }
  onClear(event) {
  ...
  onExceFromList(Month) {
    this.Month = Month
    var postData = {
      "POReports": this.POReports,
    ...
    this.commonService.postHttpService(postData, "").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
        this.generatePOdetailsExcelFormat();
        Swal.fire({
          title: 'Success!',
  ...
  generatePOdetailsExcelFormat() {
    ...
    const title = `${this.Month} Detailed SO Report`;
    const header = ["PartNo", "CAT", "InvoiceNo", "SONo", "WO", "CompanyName", "InvoiceDate", "Quantity", "Rate", "Price", "Discount", "ExtPrice", "ExtCost", "Profit", "GP", "SerialNo"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = (`${this.Month} Detailed SO Report` + currentDate + '.xlsx')
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getPurchasesByMonthReportToExcel").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
  ...
  generateExcelFormat() {
    ...
    //Excel Title, Header, Data
    const title = 'Monthly Purchase Order Report';
    const header = ["Month", "Price", "Cost", "Profit", "Margin"]
    //Create workbook and worksheet
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Data');
    // //Add Row and formatting
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      var currentDate = this.datePipe.transform(new Date(), "M-d-yyyy hh-mm-ss a")
      var filename = ('Monthly Purchase Order Report ' + currentDate + '.xlsx')
      fs.saveAs(blob, filename);
    })
