...
@Component({
  selector: 'app-po-by-item',
  templateUrl: './po-by-item.component.html',
  styleUrls: ['./po-by-item.component.scss']
})
export class PoByItemComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/POReports/PurchasesByItemReport';
    const that = this;
    var filterData = {}
    ...
    this.dataTable = $('#datatable-angular-POByItem');
  ...
  onFilter(event) {
    var table = $('#datatable-angular-POByItem').DataTable();
    ...
    table.columns(11).search(this.Status);
    table.columns(12).search(this.POType);
    table.draw();
    this._opened = !this._opened;
  }
  onClear(event) {
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getPurchasesByItemReportToExcel").subscribe(response => {
      if (response.status == true) {
  ...
  generateExcelFormat() {
    ...
    const title = 'Purchase Order By Item Report';
    const header = ["Part No", "Quantity Purchased ", "Average Amount", "Total Amount"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = ('Purchase Order By Item Report ' + currentDate + '.xlsx')
