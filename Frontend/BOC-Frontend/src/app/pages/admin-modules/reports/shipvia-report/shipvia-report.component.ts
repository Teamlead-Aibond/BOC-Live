...
@Component({
  selector: 'app-shipvia-report',
  templateUrl: './shipvia-report.component.html',
  styleUrls: ['./shipvia-report.component.scss']
})
export class ShipviaReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url3 = this.baseUrl + '/api/v1.0/RRReports/RRShipViaReport';
    const that = this;
    var filterData = {
      "CustomerId": (this.CustomerId).toString(),
      "VendorId": (this.VendorId).toString(),
      "ShipFromIdentity": this.ShipFromIdentity,
      "ShipToIdentity": this.ShipToIdentity,
      "ShipViaId": this.ShipViaId,
      "ShipDate": this.Shipdate,
      'CreatedBy':this.ShipById,
    ...
    this.dtOptions["columns"] = [
      ...
      { data: 'ShipFromName', name: 'ShipFromName', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShipToName', name: 'ShipToName', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShippedBy', name: 'ShippedBy', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShipViaName', name: 'ShipViaName', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShipDate', name: 'ShipDate', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShipStatus', name: 'ShipStatus', defaultContent: '', orderable: true, searchable: true },
      { data: 'ShipComment', name: 'ShipComment', defaultContent: '', orderable: true, searchable: true, },
      { data: 'ReceiveComment', name: 'ReceiveComment', defaultContent: '', orderable: true, searchable: true, },
      { data: 'ReceiveDate', name: 'ReceiveDate', defaultContent: '', orderable: true, searchable: true, },   
      { data: 'CreatedDate', name: 'CreatedDate', defaultContent: '', orderable: true, searchable: true, },
      { data: 'IsPickedUpType', name: 'IsPickedUpType', defaultContent: '', orderable: true, searchable: true, },
      { data: 'PickedUpDate', name: 'PickedUpDate', defaultContent: '', orderable: true, searchable: true, },
      { data: 'PickedUpBy', name: 'PickedUpBy', defaultContent: '', orderable: true, searchable: true, },
  ...
  ShipDateFormat(ShipDate) {
    ...
    let ShipDateformat = new Date(ShipDateYears, ShipDatemonths - 1, ShipDateDates);
    this.Shipdate = moment(ShipDateformat).format('YYYY-MM-DD');
  }

  onFilter(event) {
    // var table = $('#datatable-angular-ShipViaReport').DataTable();
    // table.columns(10).search(this.CustomerId);
    // table.columns(11).search(this.VendorId);
    // table.columns(12).search(this.ShipById);
    // table.columns(5).search(this.Shipdate);
    // table.columns(14).search(this.ShipFromIdentity);
    // table.columns(15).search(this.ShipToIdentity);
    // table.columns(13).search(this.ShipViaId);
    // table.draw();
    this.rerender()
    this._opened = !this._opened;
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "ShipViaExportToExcel").subscribe(response => {
