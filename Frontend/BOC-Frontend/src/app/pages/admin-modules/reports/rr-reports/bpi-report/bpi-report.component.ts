...
@Component({
  selector: 'app-bpi-report',
  templateUrl: './bpi-report.component.html',
  styleUrls: ['./bpi-report.component.scss']
})
export class BpiReportComponent implements OnInit {
  ...
  onFilterofBPIReport() {
    ...
    var url = this.baseUrl + '/api/v1.0/RRReports/BPIReport';
    const that = this;
    var filterData = {
    ...
    this.dtOptions["columns"] = [
      ...
      { data: 'Vendor', name: 'Vendor', defaultContent: '', orderable: true, searchable: true },
      ...
      { data: 'CustomerRepairCost	', name: 'CustomerRepairCost', defaultContent: '', orderable: true, searchable: true },
    ...
    this.dataTable = $('#datatable-angular-BPIReport');
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getBPIReportExportToExcel").subscribe(response => {
  ...
  generateExcelFormat() {
    ...
    const title = 'BPI Report';
    const header = ["RR No", "Part No", "Vendor Name", "Manufacturer", "Manufacturer Part No", "Serial No", "Description", "Status", "Customer Repair Cost", "Customer Name", "Price Of New", "Cost", "Shipping Cost", "Date Logged", "Quote Submitted To Customer", "Date Completed", "Is Warranty Recovery Name", "Customer PO No", "Root Cause"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = ('BPI Report ' + currentDate + '.xlsx')
