...
@Component({
  selector: 'app-openorder-by-suppplier-without-vat-report',
  templateUrl: './openorder-by-suppplier-without-vat-report.component.html',
  styleUrls: ['./openorder-by-suppplier-without-vat-report.component.scss']
})
export class OpenorderBySuppplierWithoutVatReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.IsViewEnabled) {
      ...
      this.onFilterofOpenOrderBySupplierReport();
  ...
  loadCount() {
    var postData = {};
    this.commonService.postHttpService(postData, "getOpenOrderBySupplierWithoutVatReportCount").subscribe(response => {
      if (response.status == true) {
        this.recordsTotal = response.responseData.recordsTotal;
        // $('#datatable-angular-OpenOrderBySupplierReport .dataTables_empty').text('You have '+ this.recordsTotal +' records, Please select filter to load data!');
        // this.onFilterofOpenOrderBySupplierReport();
        // $('#datatable-angular-OpenOrderBySupplierReport .dataTables_empty').text('You have '+ this.recordsTotal +' records, Please select filter to load data!');
        // $('#datatable-angular-OpenOrderBySupplierReport_filter').attr('style', 'display: none;');
      }
  ...
  onFilterofOpenOrderBySupplierReport() {
    const httpOptions = {
    ...
    var url3 = this.baseUrl + '/api/v1.0/RRReports/OpenOrderBySupplierWithoutVatReport';
    const that = this;
    var filterData = {
    ...
    this.dtOptions["columns"] = [
      ...
      { data: 'CustomerPONo', name: 'CustomerPONo', defaultContent: '', orderable: true, searchable: true, },
      { data: 'RepairPrice', name: 'RepairPrice', defaultContent: '', orderable: true, searchable: true, },
      { data: 'PriceOfNew', name: 'PriceOfNew', defaultContent: '', orderable: true, searchable: true, },
      { data: 'Cost', name: 'Cost', defaultContent: '', orderable: true, searchable: true, },
      { data: 'ShippingCost', name: 'ShippingCost', defaultContent: '', orderable: true, searchable: true, },
      { data: 'CustomerDepartmentName', name: 'CustomerDepartmentName', defaultContent: '', orderable: true, searchable: true, },
      { data: 'SONo', name: 'SONo', defaultContent: '', orderable: true, searchable: true, },
      { data: 'PONo', name: 'PONo', defaultContent: '', orderable: true, searchable: true, },
      { data: 'InvoiceNo', name: 'InvoiceNo', defaultContent: '', orderable: true, searchable: true, },
      { data: 'CriticalSpare', name: 'CriticalSpare', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'VendorInvoiceNo', name: 'VendorInvoiceNo', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'NotRepairableDate', name: 'NotRepairableDate', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'CustomerAssetName', name: 'CustomerAssetName', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'DirectedSupplier', name: 'DirectedSupplier', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'QuoteRejectCode', name: 'QuoteRejectCode', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'PODueDate', name: 'PODueDate', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'VendorRefNo', name: 'VendorRefNo', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'IsWarrantyDeniedName', name: 'IsWarrantyDeniedName', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'ShipFromCustomer', name: 'ShipFromCustomer', defaultContent: '', orderable: true, searchable: true, },
      // { data: 'ShipToName', name: 'ShipToName', defaultContent: '', orderable: true, searchable: true, },

    ];

    this.dataTable = $('#datatable-angular-OpenOrderBySupplierReport');
  ...
  getdtOption() {
    ...
    return {
      ...
      "preDrawCallback": function () {
        $('#datatable-angular-OpenOrderBySupplierReport_processing').attr('style', 'display: block; z-index: 10000 !important');
      ...
      language: {
        ...
        "loadingRecords": '&nbsp;',
        "processing": 'Loading...',
        "emptyTable": this.dtOptionsdeferLoadingShow ? '<div class="alert alert-info" style="margin-left: 29%" role="alert">Please select filter to load data!</div>' : 'No data available!'
      }
    };
  }
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getOpenOrderBySupplierWithoutVatReport").subscribe(response => {
      if (response.status == true) {
  ...
  generateExcelFormat() {
    ...
    const title = 'Open Order By Supplier Without Vat Report';
    const header = ["Part No", "Quantity", "SO #", "PO #", "Invoice #", "Rejected Date", "Completed Date", "RR No", "Supplier", "Manufacturer", "Manufacturer Part No", "Serial No", "Description", "Status", "CompanyName", "Customer PO No", "Repair Price", "Price Of New", "Cost", "Shipping Cost", "AH Received Date", "Sales Order Required Date", "Quote Submitted Date", "Approved Date", "Rush Repair", "Customer Reference 1", "Customer Reference 2", "Customer Reference 3", "Customer Reference 4", "Customer Reference 5", "Customer Reference 6", "Customer Part No 1", "Customer Part No 2", "Customer Stated Issue", "Root Cause", "Customer Department", "Follow Up Notes", "Critical Spare"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = ('Open Order By Supplier Without Vat Report ' + currentDate + '.xlsx')
