...
@Component({
  selector: 'app-openorder-by-suppplier-report',
  templateUrl: './openorder-by-suppplier-report.component.html',
  styleUrls: ['./openorder-by-suppplier-report.component.scss'],
})
export class OpenorderBySuppplierReportComponent implements OnInit {
  ...
  loadCount() {
    var postData = {};
    this.commonService.postHttpService(postData, "getOpenOrderBySupplierReportCount").subscribe(response => {
  ...
  onFilterofOpenOrderBySupplierReport() {
    ...
    var url3 = this.baseUrl + '/api/v1.0/RRReports/OpenOrderBySupplierReport';
    const that = this;
    var filterData = {
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getOpenOrderBySupplierReport").subscribe(response => {
      if (response.status == true) {
  ...
  generateExcelFormat() {
    ...
    const title = 'Open Order By Supplier Report';
    const header = ["Part No", "Quantity", "SO #", "PO #", "Vendor Bill #", "Invoice #", "Not Repairable Date", "Rejected Date", "Completed Date", "RR No", "Supplier", "Manufacturer", "Manufacturer Part No", "Serial No", "Description", "PO Due Date", "Status", "CompanyName", "Customer PO No", "Repair Price", "Price Of New", "Cost", "Shipping Cost", "AH Received Date", "Sales Order Required Date", "Quote Submitted Date", "Approved Date", "Rush Repair", "Customer Reference 1", "Customer Reference 2", "Customer Reference 3", "Customer Reference 4", "Customer Reference 5", "Customer Reference 6", "Customer Part No 1", "Customer Part No 2", "Customer Stated Issue", "Root Cause", "Customer Department", "Follow Up Notes", "Customer Asset Name", "Directed Supplier", "Quote Reject Code", "Vendor Ref No", "Warranty Denied?", "Shipped From Customer", "Ship To", "Critical Spare"]
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      var filename = ('Open Order By Supplier Report ' + currentDate + '.xlsx')
