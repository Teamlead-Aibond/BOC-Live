...
import { Invoice_Type, Invoice_Status, footerlineRight, footerlineLeft, CONST_VIEW_ACCESS, CONST_APPROVE_ACCESS, ProcessingHTML } from 'src/assets/data/dropdown';
import Swal from 'sweetalert2';
import { environment } from 'src/environments/environment';
import * as fs from 'file-saver';

@Component({
  selector: 'app-invoice-monthly-detailed-report',
  templateUrl: './invoice-monthly-detailed-report.component.html',
  styleUrls: ['./invoice-monthly-detailed-report.component.scss']
})
export class InvoiceMonthlyDetailedReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/InvoiceReports/ParticularMonthInvoiceByCustomer';
    const that = this;
    var filterData = {
      Month: this.Month,
      Year:this.Year,
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getParticularMonthInvoiceByCustomerToExcel").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
