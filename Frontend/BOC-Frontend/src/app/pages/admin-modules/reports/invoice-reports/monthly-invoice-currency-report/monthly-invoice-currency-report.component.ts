...
@Component({
  selector: 'app-monthly-invoice-currency-report',
  templateUrl: './monthly-invoice-currency-report.component.html',
  styleUrls: ['./monthly-invoice-currency-report.component.scss']
})
export class MonthlyInvoiceCurrencyReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.IsViewEnabled = this.commonService.permissionCheck("MonthlyInvoiceSummaryByCurrency", CONST_VIEW_ACCESS);
    this.DownloadExcelCSVinReports = this.commonService.permissionCheck("MonthlyInvoiceSummaryByCurrency", CONST_APPROVE_ACCESS);

    this.OverAllSummary = ''
    this.OverAllBaseSummary = ''
    this.ReportCurrencyCode = null
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/InvoiceReports/InvoiceByMonthWithCurrency';
    const that = this;
    ...
    return {
      ...
      createdRow: function (row, data, index) {
        ...
        $('.actionView1', row).bind('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          that.router.navigate(['/admin/reports/Monthly-Invoice-Detaliled-Currency-Report'], { state: { Month: data.Month, Year: data.Year, CurrencyCode: data.LCC, CreatedByLocation: data.CreatedByLocation,ReportCurrencyCode:that.ReportCurrencyCode, IncludeRR: that.IncludeRR } });
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "InvoiceByMonthReportToExcelWithCurrency").subscribe(response => {
      if (response.status == true) {
  ...
  generateExcelFormat() {
    ...
    const header = keyNames
    //["Month & Year", "Currency", "Price", "Cost", "Profit", "Margin"]
    //Create workbook and worksheet
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Data');
    // //Add Row and formatting
  ...
  onReportCurrencyCode(e) {
    this.ReportCurrencySymbol = this.CurrencyList.find(a => a.CurrencyCode = e.target.value)
  }
