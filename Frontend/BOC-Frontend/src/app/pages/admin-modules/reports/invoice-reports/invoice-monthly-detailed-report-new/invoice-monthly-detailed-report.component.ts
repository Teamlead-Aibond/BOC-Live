...
export class InvoiceMonthlyDetailedReportComponent implements OnInit {
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/InvoiceReports/ParticularMonthInvoiceByCustomerNew';
    const that = this;
    var filterData = {
      Month: this.Month,
      Year: this.Year,
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "getParticularMonthInvoiceByCustomerToExcelNew").subscribe(response => {
      if (response.status == true) {
        this.ExcelData = response.responseData.ExcelData;
