...
export class VatTaxReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.IsViewEnabled = this.commonService.permissionCheck("SOTAXReport", CONST_VIEW_ACCESS);
    this.DownloadExcelCSVinReports = this.commonService.permissionCheck("SOTAXReport", CONST_APPROVE_ACCESS);
    this.OverAllSummary = ''
  ...
  onFilterofVatTaxReport() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/SOTaxVatReport';
    const that = this;
    var filterData = {}
    ...
    this.dataTable = $('#datatable-angular-SOVatTaxReport');
  ...
  onFilter(event) {
    var table = $('#datatable-angular-SOVatTaxReport').DataTable();
    table.columns(13).search(this.CustomerId);
    table.columns(1).search(this.SONo);
    table.columns(3).search(this.InvoiceNo);
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "SOTaxVatReportToExcel").subscribe(response => {
      if (response.status == true) {
        ...
        workbook.xlsx.writeBuffer().then((data) => {
          ...
          var filename = ('SO Vat/Tax Report' + currentDate + '.xlsx')
