...
@Component({
  selector: 'app-payable-tax-report',
  templateUrl: './payable-tax-report.component.html',
  styleUrls: ['./payable-tax-report.component.scss']
})
export class PayableTaxReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.IsViewEnabled = this.commonService.permissionCheck("PayableTAXReport", CONST_VIEW_ACCESS);
    this.DownloadExcelCSVinReports = this.commonService.permissionCheck("PayableTAXReport", CONST_APPROVE_ACCESS);
    this.OverAllSummary = ''
  ...
  onList() {
    ...
    var url = this.baseUrl + '/api/v1.0/SalesOrderReports/PayableReportDetails';
    const that = this;
    var filterData = {}
    ...
    this.dtOptions["columns"] = [
      { data: 'CompanyName', width: '25%', name: 'CompanyName', defaultContent: '', orderable: true, searchable: true },

      { data: 'SONo', width: '10%', name: 'SONo', defaultContent: '', orderable: true, searchable: true },
      { data: 'SOStatusName', width: '50%', name: 'SOStatusName', defaultContent: '', orderable: true, searchable: true },
      { data: 'InvoiceNo', width: '20%', name: 'InvoiceNo', defaultContent: '', orderable: true, searchable: true },
      { data: 'InvoiceStatusName', width: '20%', name: 'InvoiceStatusName', defaultContent: '', orderable: true, searchable: true },
      { data: 'ConvertedToInvoice', width: '20%', name: 'ConvertedToInvoice', defaultContent: '', orderable: true, searchable: true },
      ...
      { data: 'PayableTax', width: '20%', name: 'PayableTax', defaultContent: '', orderable: true, searchable: true },
    ...
    this.dataTable = $('#datatable-angular-PayableVatTaxReport');
  ...
  onFilter(event) {
    var table = $('#datatable-angular-PayableVatTaxReport').DataTable();
    table.columns(16).search(this.CustomerId);
    table.columns(17).search(this.VendorId);
    table.columns(1).search(this.SONo);
    table.columns(7).search(this.PONo);
    table.columns(12).search(this.PartNo);
    table.columns(18).search(this.CustomerGroupId);
  ...
  onExcel() {
    ...
    this.spinner = true
    this.commonService.postHttpService(postData, "PayableReportDetailsToExcel").subscribe(response => {
      if (response.status == true) {
        ...
        const title = 'Payable Vat/Tax Report';
        ...
        workbook.xlsx.writeBuffer().then((data) => {
          ...
          var filename = ('Payable Vat/Tax Report' + currentDate + '.xlsx')
