import { Component, OnInit, ChangeDetectorRef, Inject, EventEmitter, ElementRef, ViewChild } from '@angular/core';
...
@Component({
  selector: 'app-outgoing-repair-tracker-side',
  templateUrl: './outgoing-repair-tracker-side.component.html',
  styleUrls: ['./outgoing-repair-tracker-side.component.scss']
})
export class OutgoingRepairTrackerSideComponent implements OnInit {
  submitted: boolean;
  ...
  title = 'Add Outgoing Repair Report'
  WorksheetValue: any;
  model: any;
  ...
  ngOnInit(): void {
    ...
    this.comments = this.localStorageService.getData('finaltestcomments');
    this.localStorageService.saveData('Worksheetoutgoing', JSON.stringify(this.Worksheet));
    const sws =  this.localStorageService.getData('selectedworksheetoutgoing');
    if(sws == null){
      this.Type = 0;
    }
    if (this.Type == 2) {
      this.findWorksheetDetails();
      this.title = 'Edit Outgoing Repair Report'
    }
    if (this.Type == 3) {
      this.findWorksheetDetails();
      this.title = 'Repair Final Report'
    }
    var selectedincomingWorksheet = this.localStorageService.getData('selectedincomingWorksheet');
      if(selectedincomingWorksheet != null){
        this.multiPDF = true;
      }else{
        this.multiPDF = false;
      }
  ...
  findWorksheetDetails() {
    const result = this.localStorageService.getData('worksheetformdataoutgoing');
    this.viewresult = this.localStorageService.getData('worksheetformdataoutgoing');
    // console.log(this.viewresult);
    const selectedworksheet = this.localStorageService.getData('selectedworksheetoutgoing');
    ...
    console.log(this.localStorageService.getData('selectedworksheetoutgoing'));
    ...
    if(selectedworksheet != '' && selectedworksheet != undefined){
      ...
      this.localStorageService.saveData('selectedoutgoingWorksheet', JSON.stringify(this.selectedWorksheet));
    }
  ...
  selectSheet(){
    ...
    if(this.WorksheetValue == undefined){
      ...
      this.outgoingWorksheetTitle = this.WorksheetValue;
      this.localStorageService.saveData('selectedworksheetoutgoing', JSON.stringify(this.WorksheetValue));
  ...
  addSave(){
    ...
    this.localStorageService.saveData('worksheetformdataoutgoing', fd);
    console.log(this.comments);
    this.localStorageService.saveData('finaltestcomments', JSON.stringify(this.comments));
  ...
  callPdf() {
    this.getPdfBase64((pdfBase64) => {
      ...
      Swal.fire({
        title: 'Success!',
        text: 'Outgoing checklist downloaded successfully!',
        type: 'success',
        confirmButtonClass: 'btn btn-confirm mt-2'
      });
   })
  ...
  getPdfBase64(cb) {
    ...
    this.commonService.getLogoAsBas64().then((base64) => {
      let pdfObj = {
        ...
        Logo: base64,
        Title: "Outgoing Checklist",
        Title1: "Part as Shipped",
        Title2: "Final Test",
...
getBothPdfBase64(cb) {
  this.spinner = true;
  this.commonService.getLogoAsBas64().then((base64) => {
    var worksheetformdataincoming = this.localStorageService.getData('worksheetformdata');
    var routecausecomments = this.localStorageService.getData('routecausecomments');
    var Worksheetincoming = this.localStorageService.getData('Worksheetincoming');
    var selectedincomingWorksheet = this.localStorageService.getData('selectedincomingWorksheet');
    var overallType = 1;
    ...
    let pdfObj = {
        ...
        incomingComments: routecausecomments ? routecausecomments : '-',

        outgoingWorksheet: this.Worksheet,
        outgoingViewresult: this.selectedWorksheet,
        outgoingComments: this.comments ? this.comments : '-'
    }
